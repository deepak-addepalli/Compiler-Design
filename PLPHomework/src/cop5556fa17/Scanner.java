		...
		while(pos < chars.length){
            ...
            switch(ch){
	            ...
	            case EOFchar :
	                if(pos != chars.length-1){
	                	new LexicalException("EOF at incorrect place", pos);
	                } else {
	            ...
	            case '/':
	                if(chars[pos+1] == '/'){
	                    int end_idx = pos+2;
	                    while(end_idx<chars.length && chars[end_idx] != '\n') {
	                        if(chars[end_idx] == '\r' && chars[end_idx+1] == '\n'){
	                        	end_idx++;
	                        	break;
	                        }
	                        else if(chars[end_idx] == '\r'){
	                        	break;
	                    	}
	                        end_idx++;
	                    }
	            ...
	            case '\"':
	                int end_Index = pos+1;
	                while(end_Index <chars.length-1 && (chars[end_Index] != '\"')) {
	                    if(chars[end_Index] == '\n' || chars[end_Index] == '\r'){
	                    	throw new LexicalException("Line terminator inside a string", end_Index);
	                    }
	                    if(chars[end_Index] == '\\' &&
	                    		(chars[end_Index+1] =='f' ||  chars[end_Index+1] == '\"' ||
	                    		chars[end_Index+1] =='n' || chars[end_Index+1] =='t' ||
	                            chars[end_Index+1] =='b' || chars[end_Index+1] =='r' ||
	                            chars[end_Index+1] =='\'' || chars[end_Index+1] =='\\') ){
	                        end_Index++;
	                    }
	                    else if(chars[end_Index] == '\\') {
	                    	throw new LexicalException("illegal escapse sequence occured", end_Index);
	                    }
	                    end_Index++;
	                }

	                if(end_Index == chars.length-1) {
	                	throw new LexicalException("String literal is not Closed properly",end_Index);
	                }
	                else {
	                    end_Index++;
	                    tokens.add(new Token(Kind.STRING_LITERAL, pos, end_Index - pos, line, posInLine));
	                    posInLine += end_Index - pos;
	                    pos = end_Index ;
	                }
	                break;
	            case '*' :
            ...
            default:
            	 if(Character.isDigit(ch)){
                     ...
                     String str = new String(chars,pos,index - pos);
                     switch(str){
                 ...
                 else throw new LexicalException("Token not detected" , pos);
            }
        }
		return this;
